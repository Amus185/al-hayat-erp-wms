...
export class InventoryRepository {
  ...
  stock() {
    return this.db.query(
      `SELECT p.name, v.sku, v.barcode, s.owner_type, s.quantity_on_hand, s.quantity_reserved,
  ...
  async stockPaginated(filters: {
    ...
    if (filters.search) {
      conditions.push(`(p.name ILIKE $${idx} OR v.sku ILIKE $${idx} OR v.barcode ILIKE $${idx})`);
    ...
    const dataSql = `
      SELECT s.id, s.variant_id, s.owner_type, s.warehouse_id, s.branch_id, s.warehouse_location_id,
  ...
  transactions() {
    return this.db.query(
      `SELECT t.*, p.name, v.sku, v.barcode
  ...
  adjust(dto: CreateAdjustmentDto, userId: string) {
    ...
    return this.db.transaction(async (client) => {
      const updated = await client.query(
        ...
        [dto.variantId, ownerType, dto.warehouseId ?? null, dto.branchId ?? null, dto.warehouseLocationId ?? null, delta]
      );
      if (updated.rowCount === 0) {
        await client.query(
          `INSERT INTO inventory_stock (variant_id, owner_type, warehouse_id, branch_id, warehouse_location_id, quantity_on_hand)
           VALUES ($1,$2,$3,$4,$5,$6)`,
          [dto.variantId, ownerType, dto.warehouseId ?? null, dto.branchId ?? null, dto.warehouseLocationId ?? null, delta]
        );
      }
      ...
      const transaction = await client.query(
        `INSERT INTO inventory_transactions
         (variant_id, transaction_type, quantity, destination_owner_type, destination_warehouse_id, destination_branch_id, destination_location_id, notes, created_by)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
        [dto.variantId, transactionType, delta, ownerType, dto.warehouseId ?? null, dto.branchId ?? null, dto.warehouseLocationId ?? null, dto.notes ?? null, userId]
      );
  ...
  lowStock() {
    return this.db.query(
      `SELECT p.name AS product_name, v.sku, v.barcode,
